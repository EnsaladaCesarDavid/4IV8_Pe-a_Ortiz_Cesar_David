...
const inputHobbieId = document.getElementById('hobbie-id');
const inputHobbieItem = document.getElementById('hobbie-item');
const inputHobbieNombre = document.getElementById('hobbie-nombre');
const inputHobbieClasificacion = document.getElementById('hobbie-clasificacion');
const formTituloHobbie = document.getElementById('form-titulo-hobbie');
const btnGuardarHobbie = document.getElementById('btn-guardar-hobbie');
const btnCancelarHobbie = document.getElementById('btn-cancelar-hobbie');
const tbodyHobbies = document.getElementById('tbody-hobbies');
const tablaHobbies = document.getElementById('tabla-hobbies');
const cargaHobbies = document.getElementById('carga-hobbies');
const contadorHobbies = document.getElementById('contador-hobbie');

const errorHobbieItem = document.getElementById('error-hobbie-item');
const errorHobbieNombre = document.getElementById('error-hobbie-nombre');
const errorHobbieClasificacion = document.getElementById('error-hobbie-clasificacion');

async function cargarHobbies() {
    try {
        const resp = await fetchAPI('/api/hobbies');
        cargaHobbies.style.display = 'none';

        if (resp.data.length === 0) {
            tablaHobbies.style.display = 'none';
            cargaHobbies.textContent = 'No hay SCPs registrados.';
            cargaHobbies.style.display = 'block';
        } else {
            tablaHobbies.style.display = 'table';
            tbodyHobbies.innerHTML = '';
            ...
            resp.data.forEach(h => {
                ...
                fila.innerHTML = `
                    ...
                    <td><span class="badge badge-${h.clasificacion}">${escapeHtml(h.clasificacion.toUpperCase())}</span></td>
                    <td>
                        <button class="btn-editar" onclick="editarHobbie(${h.id})">Editar</button>
                        <button class="btn-eliminar" onclick="confirmarEliminarHobbie(${h.id}, '${escapeHtml(h.item)}')">Eliminar</button>
                    </td>
                `;
                tbodyHobbies.appendChild(fila);
            });
        }
        contadorHobbies.textContent = `${resp.count}`;
    } catch (error) {
        mostrarNotificacion('Error al cargar SCPs: ' + error.message, 'error');
    }
}

function validarFormHobbie() {
    let ok = true;
    const item = inputHobbieItem.value.trim();
    const nombre = inputHobbieNombre.value.trim();

    if (!item || item.length < 3 || item.length > 4) {
        errorHobbieItem.textContent = 'El ítem debe tener entre 3 y 4 caracteres';
        inputHobbieItem.classList.add('input-error');
        ok = false;
    } else {
        errorHobbieItem.textContent = '';
        inputHobbieItem.classList.remove('input-error');
    }

    if (!nombre) {
        errorHobbieNombre.textContent = 'El nombre es obligatorio';
        inputHobbieNombre.classList.add('input-error');
        ok = false;
    } else {
        errorHobbieNombre.textContent = '';
        inputHobbieNombre.classList.remove('input-error');
    }

    return ok;
}

function limpiarFormHobbie() {
    formHobbie.reset();
    inputHobbieId.value = '';
    formTituloHobbie.textContent = 'Agregar SCP';
    btnGuardarHobbie.textContent = 'Guardar';
    btnCancelarHobbie.style.display = 'none';
...
formHobbie.addEventListener('submit', async (e) => {
    e.preventDefault();
    if (!validarFormHobbie()) return;

    const datos = {
        item: inputHobbieItem.value.trim(),
        nombre: inputHobbieNombre.value.trim(),
        clasificacion: inputHobbieClasificacion.value
    };
    const id = inputHobbieId.value;

    try {
        if (id) {
            await fetchAPI(`/api/hobbies/${id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(datos)
            });
            mostrarNotificacion('SCP actualizado con éxito', 'exito');
        } else {
            await fetchAPI('/api/hobbies', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(datos)
            });
            mostrarNotificacion('SCP registrado con éxito', 'exito');
        }
        limpiarFormHobbie();
        cargarHobbies();
    } catch (error) {
...
btnCancelarHobbie.addEventListener('click', limpiarFormHobbie);

async function editarHobbie(id) {
    try {
        const resp = await fetchAPI(`/api/hobbies/${id}`);
        inputHobbieId.value = resp.data.id;
        inputHobbieItem.value = resp.data.item;
        inputHobbieNombre.value = resp.data.nombre;
        inputHobbieClasificacion.value = resp.data.clasificacion;
        
        formTituloHobbie.textContent = 'Editar SCP';
        btnGuardarHobbie.textContent = 'Actualizar';
        btnCancelarHobbie.style.display = 'inline-block';
        
        formHobbie.scrollIntoView({ behavior: 'smooth' });
    } catch (error) {
        mostrarNotificacion(error.message, 'error');
    }
}

function confirmarEliminarHobbie(id, item) {
    if (confirm(`¿Estás seguro de que deseas eliminar el SCP-${item}?`)) {
        eliminarHobbie(id);
    }
}

async function eliminarHobbie(id) {
    try {
        await fetchAPI(`/api/hobbies/${id}`, { method: 'DELETE' });
        mostrarNotificacion('SCP eliminado', 'exito');
        if (inputHobbieId.value === String(id)) limpiarFormHobbie();
        cargarHobbies();
    } catch (error) {
        mostrarNotificacion(error.message, 'error');
    }
